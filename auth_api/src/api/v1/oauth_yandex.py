...
from src.services.oauth_yandex import YandexOAuthService, get_oauth_service
...
@router.get("/callback")
async def handle_yandex_oauth_callback(
    request: Request,
    ...
    try:
        ...
        # 2. Получаем по коду провайдер аксесс токен
        provider_access_token = await oauth_service.get_access_token(code)

        # 3. Получаем по провайдер аксесс токену данные юзера
        user_info = await oauth_service.get_user_info(provider_access_token)
        # user_email = user_info["email"]
        user_email = user_info.get("emails")[0]
        ...
        logger.error("Ошибка при обращении к Yandex API: %s - %s",
        ...
        logger.exception("Ошибка базы данных при Yandex OAuth:", e)
        raise HTTPException(
        ...
        logger.exception("Ошибка во время обработки Yandex OAuth:", e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Сервер временно недоступен. Попробуйте позже."
        )
